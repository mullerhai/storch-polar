package torch.polars.api

import java.util.Collections

import scala.annotation.varargs
import scala.jdk.CollectionConverters._
import torch.polars.internal.jni.data_frame
import torch.polars.api.expressions.Expression
import torch.polars.api.io.Writeable
import torch.polars.api.types.Schema
import torch.polars.config.UniqueKeepStrategies

//class DataFrame private (private[polars] val ptr: Long) {
class DataFrame ( val ptr: Long) {
  val schema: Schema = {
  ...
  def select(colName: String, colNames: String*): DataFrame =
    toLazy.select(colName, colNames*).collect(noOptimization = true)

  @varargs
  def select(column: Expression, columns: Expression*): DataFrame =
    toLazy.select(column, columns*).collect(noOptimization = true)
  ...
  def drop(colName: String, colNames: String*): DataFrame =
    toLazy.drop(colName, colNames*).collect(noOptimization = true)
...
object DataFrame {
  ...
  /** Initialize new [[DataFrame]] from one or more
    * [[Series]]. The name of a series is used as column name and its
    ...
    * @return
    * [[DataFrame]] formed from the provided
    * [[Series]]
   */
  @varargs
