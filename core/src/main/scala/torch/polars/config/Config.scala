...
import torch.polars.internal.jni.common
import torch.polars
import torch.polars.jsonMapper
...
class Config private (val options: Map[String, String]) {
  ...
  class ConfigUpdateBuilder private[config] () {
    private val options = new java.util.HashMap[String, String]()
  ...
  /** Save the config to a specified path as a JSON config string. */
  def saveTo(path: File, overwrite: Boolean) = synchronized {
...
object Config {
  ...
  private var _instance: Config = uninitialized //_
